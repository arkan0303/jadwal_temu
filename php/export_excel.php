...
$objPHPExcel->getActiveSheet()->setCellValue('H1', 'Jam');
$objPHPExcel->getActiveSheet()->setCellValue('I1', 'Tanggal');
$objPHPExcel->getActiveSheet()->setCellValue('J1', 'Jumlah Orang');
$objPHPExcel->getActiveSheet()->setCellValue('K1', 'Instansi');
$objPHPExcel->getActiveSheet()->setCellValue('L1', 'Email Pemohon');
...
if ($result) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('H' . $i, $row['jam']);
        $objPHPExcel->getActiveSheet()->setCellValue('I' . $i, $row['tanggal']);
        $objPHPExcel->getActiveSheet()->setCellValue('J' . $i, $row['jumlah_orang']);
        $objPHPExcel->getActiveSheet()->setCellValue('K' . $i, $row['instansi']);
        $objPHPExcel->getActiveSheet()->setCellValue('L' . $i, $row['email_pemohon']);
        $rowNumber++;
        $i++;
    }
}
